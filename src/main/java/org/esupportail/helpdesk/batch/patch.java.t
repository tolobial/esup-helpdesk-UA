...
import org.esupportail.helpdesk.services.navigation.TicketNavigatorForArchiving;
	...
	protected static void dispatch(final String[] args) {
		...
		case 1:
			if ("test-beans".equals(args[0])) {
				...
				deleteAllTickets();
        // rajouté par Patrick PLACIDE
  
      } else if ("archive-or-delete-before-date".equals(args[0])) {
        try {
        // Date limite : 01/06/2024
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        Date cutoffDate = sdf.parse(System.getProperty("cutoffDate", "2024-06-01"));
        
        // Mode archive ou delete
        boolean archiveMode = Boolean.parseBoolean(System.getProperty("archiveMode", "true"));

        // Récupérer les tickets avant la date
        List<Ticket> ticketsToProcess = findTicketsBeforeDate(cutoffDate);

        LOG.info("Tickets à traiter : " + ticketsToProcess.size() + ", mode archive=" + archiveMode);

        // Parcourir les tickets
        for (Ticket ticket : ticketsToProcess) {
            if (archiveMode) archiveTicket(ticket);
            else deleteTicket(ticket);
        }
        LOG.info("Traitement terminé !");
        } catch (ParseException e) {
        e.printStackTrace();
        LOG.error("Erreur : format de date invalide pour cutoffDate !");
        }
      } else {
				syntax();
			}
 ...
 */
  ...
  private static void archiveTicketsByAcademicYear(final String year) {
    TicketNavigatorForArchiving navigator =
        (TicketNavigatorForArchiving) BeanUtils.getBean(TICKET_NAVIGATOR_FOR_ARCHIVING_BEAN);
      Archiver archiver = (Archiver) BeanUtils.getBean(ARCHIVER_BEAN);

      LOG.info("Archiving tickets for academic year " + year + "...");
      navigator.setAcademicYear(year);
      archiver.run(navigator);
