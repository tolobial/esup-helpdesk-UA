...
public class DomainServiceImpl implements DomainService, InitializingBean {
	...
	public void updateTicket(final Ticket ticket) {
		//suppression des blancs en debut de champ
    ...
    @Override
public List<Ticket> getArchivableTicketsByAcademicYear(final String year) {
    List<Ticket> filtered = new ArrayList<>();

    // On recupere tous les tickets
    List<Ticket> allTickets = getTickets(0, Integer.MAX_VALUE);

    for (Ticket ticket : allTickets) {
        String ticketYear = extractAcademicYear(ticket);
        if (ticketYear != null && ticketYear.equals(year)) {
            filtered.add(ticket);
        }
    }
    return filtered;
...
@Override
public void archiveTicketsByAcademicYear(final String year) {
    List<Ticket> tickets = getArchivableTicketsByAcademicYear(year);
    if (tickets.isEmpty()) {
        logger.info("Aucun ticket a archiver pour l annee academique " + year);
        return;
    }

    for (Ticket ticket : tickets) {
        archiveTicket(ticket); // methode existante reutilisee
    
 //   String demandeur = (ticket.getCreator() != null)
 //   ? ticket.getCreator().getFirstName() + " " + ticket.getCreator().getLastName(): "Inconnu";
  
    String demandeur = (ticket.getCreator() != null)
    ? ticket.getCreator().getDisplayName() : "Inconnu";

    logger.info("Ticket #" + ticket.getId() + " (status: " + ticket.getStatus() + ")" + " - Demandeur: " + demandeur + " - Archivé");
  
  
    //logger.info("Ticket #" + ticket.getId() + " archive (annee " + year + ")");   
    }
    logger.info("Archivage termine : " + tickets.size() + " tickets pour l annee academique " + year);

}

/**
 * Deduit l annee academique d un ticket a partir de sa date de creation.
 */
private String extractAcademicYear(Ticket ticket) {
	...
	public void deleteArchivedTickets(final Integer days) {
		...
		for (ArchivedTicket archivedTicket : archivedTickets) {
			...
			if (compteur >= 1000) {
				...
				compteur = 0;
				logger.info("nb messages traites : " + compteurTotal);
			}
		}
	}
	...
	public Boolean isFaqLinksDepartment(final Category category) {
		if(category != null) {
			//cas d'une categorie directement sous le service
			if(category.getParent() == null) {
				//categorie herite des faqs du service
			...
			//cas d'une sous categorie
			} else {
				//categorie herite des faqs de la categorie parents
				if(category.getInheritFaqLinks()) {
	...
	public List<FaqLink> getInheritedFaqLinks(final Category category) {
		...
		//cas d'une categorie qui herite des faqs du service
		if (category.getInheritFaqLinks()) {
	...
	public boolean userCanViewTicket(final User user, final Ticket ticket, final List<Department> visibleDepartments) {
		...
		//controle d'accès sur les categories confidentielles
		//seules les membres de la categorie confidentielle et le proprietaire ont accès
		...
		// on verifie si le ticket appartient à un service dont la visibilite
		// inter service serait definie au sein d'un de ses services ou il est
		// manager
	...
	protected boolean userCanViewActionOrFileInfo(final User user, final Ticket ticket, final boolean invited,
		...
		// visible le user est le createur de l'action
		if (user.equals(objectUser)) {
			return true;
		}
		//si ticket publique et visibilite du commentaire publique
		if(TicketScope.PUBLIC.equals(ticket.getEffectiveScope()) && ActionScope.PUBLIC.equals(objectScope)){
			return true;
		}
		//si ticket intranet et visibilite du commentaire publique
		...
		if (ActionScope.MANAGER.equals(objectScope)) {
			// visible si user est gestionnaire du service ou si visibilite
			// inter service
			if (!isDepartmentManager(ticket.getDepartment(), user)) {
				// on verifie si le ticket appartient à un service dont la
				// visibilite inter service serait definie au sein d'un de ses
		...
		if (ActionScope.OWNER.equals(objectScope)) {
			...
			// visible si user est proprietaire du ticket
			if (user.equals(ticket.getOwner())) {
				return true;
			}
			return false;
	...
	public boolean userCanMoveBack(final User user, final Ticket ticket) {
		...
		// on prend l'action la plus recente s'il y en a
	...
	public boolean userCanInvite(final User user, final Ticket ticket) {
		...
		//cas d'un gestionnaire d'un autre service mais qui a accès via la visibilite interservice
...
//			monitoringSender.ticketMonitoringSendAlerts(author, action1, action2, false);
	...
	public void changeTicketScope(final User author, final Ticket ticket, final String ticketScope,
		...
		//cas ou l'on passe d'un ticket avec visibilite publique ou intranet a une visibilite moindre, 
		//on update les actions rattachees qui sont publiques ou intranet en bd en les passant a sujet seulement.  
